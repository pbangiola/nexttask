...
function badRequest(res,error){const message=error?.message||'Invalid request';const notFound=message.startsWith('Task node not found')||message==='Project not found';return res.status(notFound?404:400).json({error:message});}
...
router.get('/users/:userId/work-plan',(req,res)=>{try{userStore.claimUnownedTasks(req.params.userId);res.json(actionabilityStore.getTimeFitPlan(req.params.userId,req.query.availableMs,req.query.rootId||null));}catch(error){return badRequest(res,error);}});
