...
// GLOBAL CONFIGURATION: Change this to your live Render Web Service URL!
...
export default function TaskSorter() {
  ...
  // TRICK: Silent background ping to Render to trigger the wake-up cycle immediately on-access
  useEffect(() => {
  ...
  // TRICK: Load Google's Client-Side Authentication SDK script dynamically into the window canvas
  useEffect(() => {
  ...
  function initializeGoogleAuth() {
    ...
    // Set up the token client to ask for specific spreadsheet access scopes
    window.tokenClient = window.google.accounts.oauth2.initTokenClient({
      client_id: "YOUR_GOOGLE_CLIENT_ID_HERE.apps.googleusercontent.com", // Paste your real Client ID string here
  ...
  const handleGoogleLogin = () => {
    if (window.tokenClient) {
      ...
      alert("Google Identity Services script hasn't loaded yet. Try again in a second.");
    }
  };
  ...
  // Shoot export arrays out directly to our standalone Render endpoint
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 antialiased text-gray-900">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-gray-100 p-6 space-y-6">
        ...
        {/* VIEW: Interactive Comparison Screen */}
        {view === "COMPARE" && (
        ...
        {/* VIEW: Sorted Results Presentation Summary */}
        {view === "RESULTS" && (
          <div className="space-y-4">
        ...
        {/* VIEW: Deadline Target Initialization */}
        {view === "DEADLINE" && (
          <div className="space-y-4">
        ...
        {/* VIEW: Focus Workspace Tracking Panel */}
        {view === "FOCUS" && (
          <div className="space-y-4 text-center">
        ...
        {/* VIEW: Complete Operational Metric Summary */}
        {view === "COMPLETE" && (
          <div className="space-y-4 text-center">
